...
//        src[i].sending_rate
        ...
        if(i==2){
                t=-1.0*log(1-number)/(cnt);
        }

        else{
            t=-1.0*log(1-number)/(src[i].sending_rate);
        }
//        cout<<"t"<<t<<endl;
        double temp=0.0;
        for(double tot_pck=0 ; tot_pck<maxx_allowed ; tot_pck++ ){
            ...
            temp=temp+t;
//            cout<<t<<" "<<temp<<" ";
            pck[no_of_packets].s_time=temp;
            pck[no_of_packets].e_time=9999;
    ...
    while(!Q.empty()){
        ...
        if(p->type==0){
            ...
            if(p->s_time >= link_used_till[from_source]){
                p->crnt_time=p->s_time+src[from_source].source_to_switch_time;
                p->type=1;
                link_used_till[from_source]=p->crnt_time;
                Q.push(p);
            }
            else if(p->s_time + src[from_source].source_to_switch_time >= link_used_till[from_source]){
            ...
            if(p->crnt_time >= last_arrived_at_sink){
                p->e_time=p->crnt_time + swich_to_sink_time;
                last_arrived_at_sink = p->e_time;
                p->type=2;
            }
            else if(p->crnt_time + swich_to_sink_time >= last_arrived_at_sink){
                p->e_time=last_arrived_at_sink+swich_to_sink_time;
                last_arrived_at_sink=p->e_time;
                p->type=2;
            }
        }
    ...
    for(int i=0;i<no_of_packets;i++){
        ...
        delay[pck[i].packet_source]+=(pck[i].e_time-pck[i].s_time);
        dlay+=(pck[i].e_time-pck[i].s_time) ;
        total_packets++;
        //}
    }
        for(int i =0 ;  i< no_of_source ; i++){
