...
class InternetTopo(Topo):
    ...
    def __init__(self, n=2, **opts):
        ...
        nat = self.addNode('nat', cls=NAT, subnet=localSubnet,
                           inetIntf=publicIntf, localIntf=privateIntf)
        self.addLink(nat, switchS2, intfName1=publicIntf)
        self.addLink(nat, switchS1, intfName1=privateIntf, params1=natParams)
        for i in range(1, 4):
            host = self.addHost('h%d' % i, ip='192.168.0.10%d/24' %
                                i, defaultRoute='via 192.168.0.1')
